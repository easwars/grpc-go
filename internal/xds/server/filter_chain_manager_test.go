...
func (b *testCertProviderBuilder) ParseConfig(interface{}) (*certprovider.BuildableConfig, error) {
...
func newTestFilterChainManager(t *testing.T, lis *v3listenerpb.Listener) *filterChainManager {
	t.Helper()
	if lis.Address == nil {
...
func (s) TestLookup_Failures(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.desc, func(t *testing.T) {
			fci := newTestFilterChainManager(t, test.lis)
...
func (s) TestLookup_DroppedChainFallback(t *testing.T) {
	...
	fci := newTestFilterChainManager(t, lis)
