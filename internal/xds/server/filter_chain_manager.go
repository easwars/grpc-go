...
func newFilterChainManager(fcMap *xdsresource.FilterChainMap, def *xdsresource.FilterChainConfig) *filterChainManager {
...
// filter chains in a single Listener resource.
//
// For the match criteria and the filter chains, we need to use pacakge local
...
func (fcm *filterChainManager) filterChainFromConfig(config *xdsresource.FilterChainConfig) *filterChain {
	fc := &filterChain{
...
// sourcePrefixes contains source prefix related information.
...
func (fcm *filterChainManager) lookup(params lookupParams) (*filterChain, error) {
	...
	srcType := xdsresource.SourceTypeExternal
	if params.srcAddr.Equal(params.dstAddr) || params.srcAddr.IsLoopback() {
		srcType = xdsresource.SourceTypeSameOrLoopback
	}
...
func filterByDestinationPrefixes(dstPrefixes []*destPrefixEntry, isUnspecified bool, dstAddr net.IP) []*destPrefixEntry {
	...
	grpclog.Infof("easwars: in filterByDestinationPrefixes for %+v, %v, %v", dstPrefixes, isUnspecified, dstAddr)

	var matchingDstPrefixes []*destPrefixEntry
	maxSubnetMatch := noPrefixMatch
	for _, prefix := range dstPrefixes {
		grpclog.Infof("easwars: checking prefix %+v", prefix)
		if prefix.net != nil && !prefix.net.Contains(dstAddr) {
			// Skip prefixes which don't match.fc
			grpclog.Infof("easwars: prefix %+v does not match address %v", prefix, dstAddr)
			continue
		}
		...
		if matchSize < maxSubnetMatch {
			grpclog.Infof("easwars: prefix %+v is less specific than current max %v", prefix, maxSubnetMatch)
			continue
		}
		if matchSize > maxSubnetMatch {
...
func filterBySourceType(dstPrefixes []*destPrefixEntry, srcType xdsresource.SourceType) []*sourcePrefixes {
	var (
	...
	for _, prefix := range dstPrefixes {
		...
		switch srcType {
		case xdsresource.SourceTypeExternal:
			match = int(xdsresource.SourceTypeExternal)
			srcPrefix = prefix.srcTypeArr[match]
		case xdsresource.SourceTypeSameOrLoopback:
			match = int(xdsresource.SourceTypeSameOrLoopback)
			srcPrefix = prefix.srcTypeArr[match]
		}
		if srcPrefix == nil {
			match = int(xdsresource.SourceTypeAny)
			srcPrefix = prefix.srcTypeArr[match]
		}
