...
// FilterChainConfig contains the static configuration for a filter chain.
...
// FilterChainMap contains the static configuration for filter chain matching.
type FilterChainMap struct {
	// DstPrefixes is the list of destination prefix entries.
	DstPrefixes []*DestinationPrefixEntry
}

// DestinationPrefixEntry contains the matchers for a specific destination IP prefix.
type DestinationPrefixEntry struct {
	// Prefix is the destination IP prefix.
	Prefix *net.IPNet
	// SourceTypeArr contains the source type matchers.
	SourceTypeArr [3]*SourcePrefixes
}

// SourcePrefixes contains the matchers for source IP prefixes.
type SourcePrefixes struct {
	// Entries is the list of source prefix entries.
	Entries []*SourcePrefixEntry
}

// SourcePrefixEntry contains the matchers for a specific source IP prefix.
