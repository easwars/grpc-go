...
func filterChainFromProto(fc *v3listenerpb.FilterChain) (*FilterChainConfig, error) {
	hcmConfig, err := processNetworkFilters(fc.GetFilters())
	if err != nil {
		return nil, err
	}

	fcc := &FilterChainConfig{HTTPConnMgr: hcmConfig}
...
func buildFilterChainMap(fcs []*v3listenerpb.FilterChain) (FilterChainMap, error) {
	dstPrefixMap := make(map[string]*dstPrefixEntry)
	...
	for _, fc := range fcs {
		...
		if err := addFilterChainsForDestPrefixes(dstPrefixMap, fc); err != nil {
			return FilterChainMap{}, err
		}
	}

	// Check if filter chains were actually added.
	fcSeen := false
	entries := []*DestinationPrefixEntry{}
	for _, bEntry := range dstPrefixMap {
	...
	if !fcSeen {
		return FilterChainMap{}, nil
	}
	return FilterChainMap{DstPrefixes: entries}, nil
}

func addFilterChainsForDestPrefixes(dstPrefixMap map[string]*dstPrefixEntry, fc *v3listenerpb.FilterChain) error {
	...
	if len(dstPrefixes) == 0 {
		...
		if dstPrefixMap[unspecifiedPrefixMapKey] == nil {
			dstPrefixMap[unspecifiedPrefixMapKey] = &dstPrefixEntry{entry: &DestinationPrefixEntry{}}
		}
		return addFilterChainsForServerNames(dstPrefixMap[unspecifiedPrefixMapKey], fc)
	}
	for _, prefix := range dstPrefixes {
		p := prefix.String()
		if dstPrefixMap[p] == nil {
			dstPrefixMap[p] = &dstPrefixEntry{entry: &DestinationPrefixEntry{Prefix: prefix}}
		}
		if err := addFilterChainsForServerNames(dstPrefixMap[p], fc); err != nil {
			return err
		}
	}
	return nil
...
func addOrCreateSourcePrefixEntry(srcPrefixes *SourcePrefixes, prefix *net.IPNet, fc *v3listenerpb.FilterChain) error {
	...
	if entry == nil {
		entry = &SourcePrefixEntry{
			Prefix:  prefix,
			PortMap: make(map[int]*FilterChainConfig),
		}
