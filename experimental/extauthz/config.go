...
type HeaderMutationRules struct {
	...
	// is disallowed. If true, the data plane RPC will be failed with a 500
	// error. Whether this maps to UNKNOWN or INTERNAL still needs to be
	// decided.
	DisallowIsError bool
}

// Apply takes a set of header mutations from an authorization server and applies
// them to the provided metadata, subject to the rules defined in hmr.
//
...
func (hmr *HeaderMutationRules) Apply(hvos []*v3corepb.HeaderValueOption, input metadata.MD) (output metadata.MD, err error) {
	if hmr == nil {
